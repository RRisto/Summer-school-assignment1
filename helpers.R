...
makeTermTable=function(dtm) {
  D <- dtm$nrow  # number of documents 
  W <- dtm$ncol  # number of terms in the vocab 
  # number of tokens per document 
  doc.length <- rowSums(as.matrix(dtm)) 
  N <- sum(doc.length)  # total number of tokens in the data
  # frequencies of terms in the corpus 
  term.table <- colSums(as.matrix(dtm))
  term.table <- sort(term.table, decreasing = TRUE)
  term.table
}

#term frequency
termFrequency=function(term.table, minFreq=5) {
  vocab <- names(term.table)
  term.frequency <- as.integer(term.table)  
  data.frame(term=vocab, frequency=term.frequency)
}
############helpers for NYT data gathering
